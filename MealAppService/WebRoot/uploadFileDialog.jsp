...
var param = {
		...
		title : ($("#txttitle").val()),
		intro : ($("#txtintro").val()),
		typeid : ($("#DropDownList1").val()),
		businessid : $("#DropDownList1").val(),
		img_url:$("#div_guid").html(),
		price : ($("#txtprice").val()),
		amount : ($("#txtamount").val())
	};
	$.post("servlet/ServletService?ran=" + Math.random(), param, function(data) {
		if (data.length > 0) {	
			if(data==1){
			alert("操作成功");
	...
	$.get("servlet/ServletService?ran=" + Math.random(), param, function(data) {	
		if (data.length > 0) {
			...
			if (data.length > 0) {
				...
				$("#DropDownList1").val(data[0].businessid) ;
			...
			<div style=" padding:10px;">
				...
				<table border="0" class="fileTable">
					<tr>
						...
						<td class="td-right"><input id="txttitle" style="width:260px;" class="textbox"
